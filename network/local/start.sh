...
# create the scchannel. Specify the channel name and the profile to be used.
./network.sh createChannel -c scchannel

# deploy chaincode scchaincode on scchannel

# Uncomment if chaincode initialization required and add "Initialize" function to the chaincode.
# ./network.sh deployCC -c scchannel -ccn scchaincode -ccp ../../chaincode -ccl go -cci Initialize
./network.sh deployCC -c scchannel -ccn scchaincode -ccp ../../chaincode -ccl go -ccs $SEQ_VAL
